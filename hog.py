...
def next_perfect_square(score):
    iteration = score
    isItSquare = False
    while isItSquare == False:
        iteration += 1
        if perfect_square(iteration) == True:
            isItSquare == True
            return iteration
...
def is_always_roll(strategy, goal=GOAL):
    ...
    iterator = 0
    always = True
    while counter < 100 and always:
            if strategy(counter, iterator) != strategy(0, 0):
                always = False
            else:
                if iterator == 99:
                    counter += 1
...
def make_averaged(original_function, total_samples=1000):
    ...
    def lower_function(*args):
        ...
        while counter != 0:
            ...
            counter -= 1
        return round(total / total_samples, 2)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    checker = 0
    while counter < 11:
        final = make_averaged(roll_dice, total_samples)(counter, dice)
        if final > average:
            average = final
            checker = counter
        counter += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    total = threshold + score
    if tail_strategy(score, opponent_score, threshold, num_rolls) == 0 or square_update(0, score, opponent_score, dice=six_sided) >= total:
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
